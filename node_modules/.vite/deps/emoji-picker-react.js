...
function unifiedWithoutSkinTone(unified) {
	...
	if (skinTonesMapped[skinTone]) return splat.join("-");
	return unified;
}
...
function visibleEmojiOneRowUp(element) {
	...
	if (indexInRow > lastIndexInRow) return allPrevEmojis.at(-1);
...
function preloaEmojidIfNeeded(emoji, emojiStyles$1, scrollTop, clientHeight, topOffset, style, dimensions, getEmojiUrl) {
	if (!emoji) return;
...
function lastVisibleEmoji(element) {
	if (!element) return null;
	var _allEmojis$slice = allVisibleEmojis(element).slice(-1), last = _allEmojis$slice[0];
	if (!last) return null;
...
function firstVisibleEmoji(parent) {
	...
	var allEmojis$1 = allVisibleEmojis(parent);
	return firstVisibleElementInContainer(parent, allEmojis$1, .1);
}
...
function useCategoryHeight(emojiCount) {
	...
	var computeAndSetDimensions = (0, import_react.useCallback)(function() {
		...
		var categoryHeight = Math.ceil(emojiCount / emojisPerRow) * emojiSize;
		setDimensions({
			categoryHeight,
			emojisPerRow,
			emojiSize
	...
	(0, import_react.useEffect)(function() {
		...
		function onMouseOver(e) {
			...
			if (button) {
				...
				if (belowFoldByPx < buttonHeight) return handlePartiallyVisibleElementFocus(button, setPreviewEmoji);
				focusElement(button);
			}
		}
		return function() {
...
function useActiveCategoryScrollDetection(setActiveCategory) {
	...
	(0, import_react.useEffect)(function() {
		...
		var observer = new IntersectionObserver(function(entries) {
			...
			if (lastCategory[1] == 1) return setActiveCategory(lastCategory[0]);
...
//#endregion
export { Categories, ExportedEmoji as Emoji, EmojiStyle, SkinTonePickerLocation, SkinTones, SuggestionMode, Theme, emoji_picker_react_esm_default as default };
